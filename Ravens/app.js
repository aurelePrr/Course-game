...
class Particles {
    ...
    update(){
        this.x += this.speedX
        this.radius += 0.3
        if(this.radius > this.maxRadius - 5) this.markedForDeletion = true // LE -5 PERMET D EVITER LA REAPARITION DE LA BULLE DE TEMPS A AUTRE
    }
    draw(){
        ctx.save()
        ctx.globalAlpha = 1 - this.radius/this.maxRadius // PERMET QUE LES PARTICULES DEVIENNENT PETIT A PETIT TRANSPARENTES
        ctx.beginPath()
...
function drawScore() {
    ...
    ctx.fillStyle = 'white'
    ctx.fillText('Score : ' + score, 58, 80)
}
